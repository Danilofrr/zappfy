...
ALTER TABLE public.delivery_tracking_settings
  ...
  ADD COLUMN IF NOT EXISTS vehicle_color text NOT NULL DEFAULT 'verde',
  ADD COLUMN IF NOT EXISTS vehicle_custom_url text,
  ADD COLUMN IF NOT EXISTS pin_color text NOT NULL DEFAULT 'verde',
  ADD COLUMN IF NOT EXISTS pin_custom_url text,
  ADD COLUMN IF NOT EXISTS timeline_color text NOT NULL DEFAULT '#10b981',
  ADD COLUMN IF NOT EXISTS card_color text NOT NULL DEFAULT '#0b1220',
  ADD COLUMN IF NOT EXISTS msg_aguardando text NOT NULL DEFAULT 'Recebemos seu pedido e já estamos preparando tudo.',
  ADD COLUMN IF NOT EXISTS msg_preparando text NOT NULL DEFAULT 'Seu pedido está sendo preparado com carinho.',
  ADD COLUMN IF NOT EXISTS msg_saiu text NOT NULL DEFAULT 'Seu pedido já saiu para entrega e está a caminho.',
  ADD COLUMN IF NOT EXISTS msg_chegando text NOT NULL DEFAULT 'Seu entregador está próximo do destino.',
  ADD COLUMN IF NOT EXISTS msg_entregue text NOT NULL DEFAULT 'Pedido entregue com sucesso. Obrigado pela preferência.',
  ADD COLUMN IF NOT EXISTS msg_cancelado text NOT NULL DEFAULT 'Este pedido foi cancelado.';
