...
ALTER TABLE public.delivery_tracking_settings
  ...
  ADD COLUMN IF NOT EXISTS show_product_price boolean NOT NULL DEFAULT true;
...
BEGIN
  ...
  -- Enrich items with product image_url when available
  SELECT COALESCE(jsonb_agg(
    CASE
      WHEN p.image_url IS NOT NULL THEN item || jsonb_build_object('image_url', p.image_url)
      ELSE item
    END
  ), '[]'::jsonb)
  INTO enriched_items
  ...
  result := jsonb_build_object(
    ...
    'settings', jsonb_build_object(
      ...
      'show_products', COALESCE(s.show_products, true),
