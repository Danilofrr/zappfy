CREATE OR REPLACE FUNCTION public.submit_public_order(_slug text, _customer text, _phone text, _cep text, _address text, _reference text, _district text, _city text, _product_id uuid, _quantity integer, _unit_price numeric, _shipping_value numeric, _total numeric, _payment text, _notes text DEFAULT ''::text)
 RETURNS uuid
 LANGUAGE plpgsql
 SECURITY DEFINER
