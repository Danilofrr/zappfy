...
CREATE POLICY "anon public checkout insert orders"
ON public.orders
FOR INSERT
TO anon
WITH CHECK (
...
BEGIN
  ...
  END IF;
  IF COALESCE(trim(_customer), '') = '' THEN
    RAISE EXCEPTION 'Nome do cliente não preenchido';
  END IF;
  IF COALESCE(trim(_phone), '') = '' THEN
    RAISE EXCEPTION 'WhatsApp inválido';
  END IF;
  IF COALESCE(trim(_address), '') = '' THEN
    RAISE EXCEPTION 'Endereço não preenchido';
  END IF;
  IF _safe_quantity < 1 THEN
    RAISE EXCEPTION 'Quantidade inválida';
  END IF;
  IF _safe_unit_price < 0 THEN
    RAISE EXCEPTION 'Valor unitário inválido';
  END IF;
  IF _safe_shipping_value < 0 THEN
    RAISE EXCEPTION 'Valor de entrega inválido';
  END IF;
  IF _safe_total <= 0 THEN
    RAISE EXCEPTION 'Valor total inválido';
  END IF;
  IF _payment NOT IN ('pix', 'cartao', 'dinheiro') THEN
    RAISE EXCEPTION 'Forma de pagamento não selecionada';
  END IF;
  ...
  END IF;
  IF COALESCE(_product.stock, 0) < _safe_quantity THEN
  ...
  _combined_notes := NULLIF(concat_ws(E'\n', NULLIF(_notes, ''), NULLIF('CEP: ' || COALESCE(_cep, ''), 'CEP: '), NULLIF('Ponto de referência: ' || COALESCE(_reference, ''), 'Ponto de referência: ')), '');
...
GRANT EXECUTE ON FUNCTION public.submit_public_order(text, text, text, text, text, text, text, text, uuid, integer, numeric, numeric, numeric, text, text) TO anon;
GRANT EXECUTE ON FUNCTION public.submit_public_order(text, text, text, text, text, text, text, text, uuid, integer, numeric, numeric, numeric, text, text) TO authenticated;
GRANT EXECUTE ON FUNCTION public.submit_public_order(text, text, text, text, text, text, text, text, uuid, integer, numeric, numeric, numeric, text, text) TO service_role;