...
-- RPC to persist a geocoded destination from the public tracking page
CREATE OR REPLACE FUNCTION public.set_tracking_destination(_code text, _lat double precision, _lng double precision)
RETURNS boolean
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  UPDATE public.delivery_tracking
    SET delivery_latitude = _lat, delivery_longitude = _lng
    WHERE id = t.id;
  RETURN true;
END;
$$;

GRANT EXECUTE ON FUNCTION public.set_tracking_destination(text, double precision, double precision) TO anon, authenticated;

-- Include destination coords in the public payload
