ALTER TABLE public.delivery_tracking
  ...
  ADD COLUMN IF NOT EXISTS delivery_geocoding_status text NOT NULL DEFAULT 'pending';

CREATE OR REPLACE FUNCTION public.set_tracking_destination(
  _code text,
...
BEGIN
  ...
  safe_status := CASE
    WHEN _status IN ('pending', 'success', 'failed', 'manual') THEN _status
    ELSE 'success'
  END;

  IF t.delivery_latitude IS NOT NULL AND t.delivery_longitude IS NOT NULL THEN
    UPDATE public.delivery_tracking
      SET delivery_geocoded_address = COALESCE(delivery_geocoded_address, NULLIF(_address, '')),
          delivery_geocoding_status = COALESCE(NULLIF(delivery_geocoding_status, ''), safe_status)
      WHERE id = t.id;
    RETURN true;
  END IF;
  ...
  IF _lat IS NULL OR _lng IS NULL THEN
    UPDATE public.delivery_tracking
      SET delivery_geocoded_address = COALESCE(NULLIF(_address, ''), delivery_geocoded_address),
  ...
  UPDATE public.delivery_tracking
    SET delivery_latitude = _lat,
        ...
        delivery_geocoded_address = COALESCE(NULLIF(_address, ''), delivery_geocoded_address),
        delivery_geocoding_status = safe_status
    WHERE id = t.id;
  RETURN true;
END;
$function$;
...
BEGIN
  ...
  result := jsonb_build_object(
    ...
    'order', jsonb_build_object(
      ...
      'status', o.status, 'date', o.date, 'notes', o.notes, 'items', o.items
    ),
