...
import type { PaymentMethod, Product, Settings, ShippingOption, Order } from "@/lib/store";
...
export type CheckoutOrderInput = Omit<Order, "id">;
...
export function CheckoutView({
  ...
  products: Product[];
  settings: Settings;
  onSubmit: (order: CheckoutOrderInput) => Promise<void>;
}) {
      ...
      <main className="mx-auto max-w-3xl px-4 sm:px-6 py-8">
        {noProducts ? (
          <div className="p-10 text-center" style={cardStyle}>
            ...
            <p className="mt-1 text-sm opacity-70">A loja ainda não cadastrou produtos.</p>
          </div>
        ) : (
