...
type Payload = {
  ...
  order: { id: string; customer: string; address: string; district: string; city: string; total: number; date: string };
...
function RastreioPage() {
  ...
  // best-effort geocode of destination address (Nominatim)
  useEffect(() => {
  ...
  const message =
    data.status === "entregue" ? s.delivered_message : data.status === "preparando" || data.status === "aguardando_motoboy" ? s.welcome_message : info.message;
  ...
  return (
    ...
    >
      ...
      <header className="px-5 pt-8 pb-6 text-center" style={{ background: `linear-gradient(180deg, ${s.secondary_color}, transparent)` }}>
        ...
        <div className="mt-3 inline-flex items-center gap-2 px-3 py-1 rounded-full text-sm font-medium" style={{ background: `${s.primary_color}22`, color: s.primary_color }}>
      ...
      <main className="max-w-md mx-auto px-4 space-y-4">
        ...
        <section
          ...
          style={{ background: s.secondary_color, border: `1px solid ${s.primary_color}33` }}
        >
        ...
        {!isFinished && (
          <section className="rounded-2xl overflow-hidden shadow-xl" style={{ border: `1px solid ${s.primary_color}33` }}>
            {courierPos ? (
              ...
              <div className="h-[260px] flex items-center justify-center text-sm opacity-70" style={{ background: s.secondary_color }}>
        ...
        {!isFinished && (
          <section className="grid grid-cols-2 gap-3">
            {s.show_distance && (
              <InfoCard color={s.primary_color} bg={s.secondary_color} label="Distância" value={distance != null ? formatDistance(distance) : "—"} />
            )}
            {s.show_estimated_time && (
              <InfoCard color={s.primary_color} bg={s.secondary_color} label="Chegada estimada" value={eta != null ? `${eta} min` : "—"} />
            )}
            <InfoCard color={s.primary_color} bg={s.secondary_color} label="Última atualização" value={formatRelative(data.last_updated_at)} />
            {s.show_courier_name && data.courier.name && (
              <InfoCard color={s.primary_color} bg={s.secondary_color} label="Entregador" value={data.courier.name} />
            )}
          </section>
        )}

        {/* Timeline */}
        <section className="rounded-2xl p-5" style={{ background: s.secondary_color, border: `1px solid ${s.primary_color}22` }}>
          <h3 className="text-sm font-semibold mb-4">Acompanhamento</h3>
          <Timeline status={data.status} primary={s.primary_color} />
        </section>

        {/* Address */}
        <section className="rounded-2xl p-4 text-sm" style={{ background: s.secondary_color, border: `1px solid ${s.primary_color}22` }}>
      ...
      {TIMELINE_STEPS.map((step, i) => {
        ...
        return (
          <li key={step.key} className="relative">
            <span
              className="absolute -left-[26px] top-1 h-5 w-5 rounded-full flex items-center justify-center"
              style={{
                background: done ? primary : "transparent",
                border: `2px solid ${done ? primary : `${primary}55`}`,
                boxShadow: isCurrent ? `0 0 0 6px ${primary}33` : "none",
              }}
            >
              {done && <Check className="h-3 w-3 text-white" />}
            </span>
            <div className={`text-sm ${done ? "font-semibold" : "opacity-60"}`} style={done ? { color: primary } : {}}>
              {step.label}
            </div>
          </li>
        );
