...
function CourierPage() {
  ...
  return (
    <div className="min-h-screen bg-[#0b1220] text-white pb-10">
      <header className="px-5 pt-6 pb-5 text-center border-b border-white/5 bg-gradient-to-b from-emerald-900/30 to-transparent">
        {data.store.logo_url && (
          <img src={data.store.logo_url} alt={data.store.name} className="h-12 w-auto mx-auto mb-2 object-contain" />
        )}
        <div className="text-lg font-bold">{data.store.name}</div>
        <div className="text-xs opacity-70">Pedido #{orderShortNumber(data.order.id)}</div>
        <div className={`mt-3 inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-sm font-medium ${info.bg} ${info.color}`}>
      ...
      <main className="max-w-md mx-auto px-4 space-y-4 mt-4">
        ...
        <section className="rounded-2xl bg-white/5 border border-white/10 p-4">
          <div className="flex items-start gap-2">
            ...
            <div className="flex-1">
              ...
              <div className="font-semibold leading-snug">{fullAddress}</div>
              {data.order.notes && (
          ...
          <a
            ...
            target="_blank"
            rel="noreferrer"
            className="mt-3 w-full inline-flex items-center justify-center gap-1.5 rounded-xl bg-blue-600 hover:bg-blue-700 px-3 py-2.5 text-sm font-semibold"
          >
        ...
        <section className="rounded-2xl bg-white/5 border border-white/10 p-4 space-y-2">
          ...
          <div className="font-semibold">{data.order.customer}</div>
          {data.order.phone && (
            <a
          ...
          {data.notes && (
            <div className="text-xs italic opacity-80 pt-1 border-t border-white/5">Obs: {data.notes}</div>
          )}
        </section>
        ...
        {!isFinished && (
          <section className="rounded-2xl bg-white/5 border border-white/10 p-4 space-y-3">
            ...
            {watching && (
              <div className="rounded-lg bg-emerald-500/10 text-emerald-300 text-xs p-3">
            ...
            {data.status === "aguardando_motoboy" || data.status === "preparando" ? (
              <Button onClick={handleStart} className="w-full h-12 text-base bg-emerald-600 hover:bg-emerald-700">
                <Bike className="h-5 w-5 mr-2" /> Iniciar Entrega
              </Button>
            ) : (
              <>
                {!watching && (
                  <Button onClick={startWatch} className="w-full h-11 bg-emerald-600 hover:bg-emerald-700">
                    <MapPin className="h-4 w-4 mr-2" /> Retomar rastreamento
                  </Button>
                )}
                {data.status !== "chegando" && (
                  <Button onClick={() => changeStatus("chegando")} className="w-full h-11 bg-purple-600 hover:bg-purple-700">
                    Estou chegando
                  </Button>
                )}
                <Button onClick={() => changeStatus("entregue")} className="w-full h-12 text-base bg-blue-600 hover:bg-blue-700">
        ...
        {isFinished && (
          <section className="rounded-2xl bg-emerald-500/10 border border-emerald-500/30 p-6 text-center">
            <CheckCircle2 className="h-10 w-10 text-emerald-400 mx-auto mb-2" />
            <div className="text-lg font-semibold">{data.status === "entregue" ? "Entrega finalizada" : "Entrega cancelada"}</div>
            {data.completed_at && (
      ...
      <footer className="text-center text-[11px] opacity-40 mt-8">Powered by Zappfy</footer>
    </div>
  );
}
